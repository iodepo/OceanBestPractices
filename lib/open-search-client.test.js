...
describe('open-search-client', () => {
  beforeAll(() => {
    // SignatureV4 looks for AWS credentials so this fails when run
    // on Github. We can just mock it out and return the request because
    // we mock the actual request response anyway.
    ...
    signer.sign = jest.fn().mockImplementation((request) => (request));
  });

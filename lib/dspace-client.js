...
/**
   ...
   * @returns {Promise<Object[]>} List of DSpace items that match the query.
   */
  find: async (endpoint, key, value) => got.post(`${endpoint}/rest/items/find-by-metadata-field?expand=metadata,bitstreams`, {
    json: {
      key,
      value,
    },
   ...
   *
   * @param {string} endpoint DSpace endpoint. Should include protocol.
   * @returns {Promise<string>} DSpace RSS feed in XML format
   */
  getFeed: async (endpoint) => got(`${endpoint}/feed/rss_2.0/site`, {
    headers: {
      ...headers,
   ...
   * @param {string} [searchParams.expand='none'] Metadata fields to
   *  expand in the query. Valid options include 'metadata', 'bitstreams', or
   * 'none'.
   ...
   * paginating results.
   *
   * @returns {Promise<Object[]>} List of DSpace items.
   */
  getItems: async (endpoint, searchParams = {}) => {
    ...
    return got(`${endpoint}/rest/items`, {
      headers,
      searchParams: {
        expand,
        limit,
   ...
   * @returns {Promise<unknown|undefined>} DSpace item or null if not found.
   */
  getItem: async (endpoint, uuid) => {
    try {
      return await got(`${endpoint}/rest/items/${uuid}`, {
   ...
   * @param {string} uuid DSpace item UUID.
   *
   * @returns {Promise<unknown|undefined>} Metadata for the DSpace item
   * or undefined if the item could not be found.
   */
  getMetadata: async (endpoint, uuid) => {
    try {
      return await got(`${endpoint}/rest/items/${uuid}/metadata`, {
        headers,
        responseType: 'json',
        resolveBodyOnly: true,
      });

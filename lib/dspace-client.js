...
/**
   ...
   */
  find: async (endpoint, key, value) => {
    const items = await got.post(`${endpoint}/rest/items/find-by-metadata-field?expand=metadata,bitstreams`, {
      json: {
        key,
        value,
      },
    ...
    return items.map((i) => new DSpaceItem(i));
  },
  ...
  getItems: async (endpoint, searchParams = {}) => {
    ...
    const items = await got(`${endpoint}/rest/items`, {
      headers,
      searchParams: {
        expand,
        limit,
   ...
   * @returns {Promise<DSpaceItem|undefined>} DSpace item or null if not found.
   */
  getItem: async (endpoint, uuid) => {
    try {
      const item = await got(`${endpoint}/rest/items/${uuid}`, {

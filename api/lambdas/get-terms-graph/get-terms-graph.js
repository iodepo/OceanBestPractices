...
function graphTerm(term, callback) {
  ...
  executeQuery(buildTermIRIQuery(term), function(err, results) {
    if (err === null) {
...
function graphTermURI(termURI, callback) {
  ...
  if (termURI.includes("purl.obolibrary.org") || termURI.includes("purl.unep.org")) {
    executeQuery(buildSuperTermQuery(termURI), function(err, supers) {
      if (err === null) {
        executeQuery(buildUsesInOntologyQuery(termURI), function(err, uses) {
          if (err === null) {
            const graph = { "parents": parseSuperTermResponse(supers), "children": parseUsesInOntologyResponse(uses) };
            callback(null, { statusCode: 200, body: JSON.stringify(graph), headers: responseHeaders() });  
          } else {
            respondWithError(err, callback);
          }
        });
    ...
    executeQuery(buildSKOSGraphQuery(termURI), function(err, relations) {
      if (err === null) {
...
function executeQuery(query, callback) {
  http.get(queryOpts(query), function(res) {
    var body = '';
...
function buildUsesInOntologyQuery(termIRI) {
  ...
  query = query + " SELECT DISTINCT ?ref ?label ?rell";
  query = query + " FROM " + ontGraph;
  query = query + " WHERE { ?ref ?refp ?o . FILTER ( ?refp IN ( owl:equivalentClass, rdfs:subClassOf ) ) . OPTIONAL { ?ref rdfs:label ?label } . OPTIONAL { ?o owl:onProperty ?rel . ?rel rdfs:label ?rell } .";
  query = query + " {";
  query = query + " {";
  query = query + " SELECT ?s ?p ?o FROM " + ontGraph;
  query = query + " WHERE { ?o ?p ?s . FILTER ( ?p IN ( rdf:first, rdf:rest, owl:intersectionOf, owl:unionOf, owl:someValuesFrom, owl:hasValue, owl:allValuesFrom, owl:complementOf, owl:inverseOf, owl:onClass, owl:onProperty ) ) } }";
...
function parseSKOSGraphResponse(results) {
  ...
  results.forEach((r) => {
    if (r.narrower !== undefined) {
      ...
      graph["parents"]["narrower"].push({ "uri": r.narrower.value, "label": r.narrowerLabel.value });
      ...
      graph["children"]["broader"].push({ "uri": r.broader.value, "label": r.broaderLabel.value });
      ...
      graph["siblings"]["related"].push({ "uri": r.related.value, "label": r.relatedLabel.value });
    }
  });
...
function parseUsesInOntologyResponse(results) {
  ...
  results.forEach((r) => {
    ...
    graph[r.rell.value].push({ "uri": r.ref.value, "label": r.label.value });
  });
...
function parseSuperTermResponse(results) {
  ...
  results.forEach((r) => {
    ...
    if (r.superTerm.value.match(/^nodeID/) && r.relLabel !== undefined) {
      if (graph[r.relLabel.value] === undefined) {
        graph[r.relLabel.value] = [];
      }

      graph[r.relLabel.value].push({ "label": r.refLabel.value, "uri": r.ref.value });
      ...
      graph["is a"].push({ "label": r.superLabel.value, "uri": r.superTerm.value });
    }

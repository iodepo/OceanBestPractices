...
/**
   ...
   *  expand in the query. Valid options include 'metadata' or 'bitstreams'.
   ...
   * @returns {Promise<Object|undefined>} DSpace item or null if not found.
   */
  getItem: async (endpoint, uuid) => {
    try {
      return got(`${endpoint}/rest/items/${uuid}`, {
   ...
   * @param {string} uuid DSpace item UUID.
   *
   * @returns {Promise<Object|undefined>} Metadata for the DSpace item
   * or undefined if the item could not be found.
   */
  getMetadata: async (endpoint, uuid) => {
    try {
      return got(`${endpoint}/rest/items/${uuid}/metadata`, {
        headers: this.headers,
        responseType: 'json',
        resolveBodyOnly: true,
      });

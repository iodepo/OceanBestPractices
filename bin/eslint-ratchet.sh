...
if [ "$PROBLEMS" -lt "$RATCHET" ]; then
  if [ "$CI" = "true" ]; then
    echo "eslint problems decreased from ${PROBLEMS} to ${RATCHET}, but .eslint-ratchet was not updated." >&2
    exit 1
  else
    echo "eslint problems decreased from ${PROBLEMS} to ${RATCHET}. Ratcheting down."
